...
struct LxcDetailView: View {
    let initialContainer: ProxmoxContainer
    let serverAddress: String
    let onBack: () -> Void

    @StateObject private var viewModel: LxcDetailViewModel
    ...
    init(container: ProxmoxContainer, serverAddress: String, onBack: @escaping () -> Void) {
        ...
        _viewModel = StateObject(
            wrappedValue: LxcDetailViewModel(container: container, serverAddress: serverAddress)
        )
    }
    ...
    var body: some View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    LxcMetricsGrid(
                        ...
                        vmCPUs: viewModel.container.cpus
                    )

                    LxcUptimeCard(uptimeSeconds: viewModel.displayedUptime)
        ...
        } message: { Text("Send ACPI reboot to \(viewModel.container.name).") }
        ...
        .alert("Shut down Container?", isPresented: $showShutdownConfirm) {
            ...
            Button("Cancel", role: .cancel) { }
        } message: { Text("Send ACPI shutdown to \(viewModel.container.name).") }
        .alert("Force stop Container?", isPresented: $showForceStopConfirm) {
            ...
            Button("Cancel", role: .cancel) { }
        } message: { Text("Immediately power off \(viewModel.container.name). Data loss possible.") }
        ...
        .sheet(isPresented: $showEditResources) {
            EditLxcResourcesSheet(viewModel: viewModel)
