...
struct ContainersView: View {
  ...
  @State private var selectedProxmoxContainer: ProxmoxContainer? = nil
  ...
  var body: some View {
    NavigationStack {
      ZStack {
        ...
        if let selected = selectedProxmoxContainer {
          ContainerDetailView(
            container: selected,
            serverAddress: storedServerAddress,
            onBack: { selectedProxmoxContainer = nil }
          )
        } else if viewModel.isLoading && viewModel.containers.isEmpty {
          ScrollView {
            VStack(spacing: 12) {
              ProgressView()
          ...
          ScrollView {
            VStack(alignment: .leading, spacing: 8) {
              Text("No Containers found")
                .font(.subheadline.weight(.semibold))
              Text("Pull down to refresh or check that you have permissions to view Containers.")
                .font(.footnote)
                .foregroundStyle(.secondary)
            }
            .frame(maxWidth: .infinity, alignment: .leading)
            ...
            VStack(alignment: .leading, spacing: 24) {
              ...
              VStack(spacing: 12) {
                ForEach(viewModel.containers) { container in
                  Button(action: { selectedProxmoxContainer = container }) {
      ...
      .refreshable {
        if selectedProxmoxContainer == nil {
          await viewModel.refresh()
        }
      }
      .onChange(of: selectedProxmoxContainer?.id) { _, newId in
        if newId != nil {
    ...
    VStack(alignment: .leading, spacing: 8) {
      HStack(spacing: 8) {
        ...
        VStack(alignment: .leading, spacing: 2) {
          Text("LXC Containers").font(.title2.bold())
          Text("Manage your containers.")
            .font(.subheadline)
            .foregroundStyle(.secondary)
        }
      }
  ...
  private func containerCard(_ container: ProxmoxContainer) -> some View {
    ...
    return VStack(alignment: .leading, spacing: 12) {
      HStack(alignment: .center, spacing: 10) {
        if let imageName = distroImageName(from: container.tags) {
          Image(imageName)
            .resizable()
            .scaledToFill()
