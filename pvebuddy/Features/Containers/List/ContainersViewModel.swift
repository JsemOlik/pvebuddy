...
final class ContainersViewModel: ObservableObject {
    ...
    func refresh() async {
        ...
        do {
            let fetchedContainers = try await client.fetchAllContainers()
            
            // Filter by selected node if one is chosen
            if let node = selectedNode, !node.isEmpty {
                self.containers = fetchedContainers.filter { $0.node == node }
            } else {
            ...
            case .noNodesFound:
                errorMessage = "No Containers were returned by the Proxmox API. Check that your token has permission to view Containers."
            }
        } catch {
